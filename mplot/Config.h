...
namespace mplot {
    ...
    public:
         ...
         *
         * e.g. mplot_program -co:varname=43 -co:"stringvar=something with spaces"
         *
        ...
        void process_args (int argc, char **argv)
        {
            for (int i = 0; i < argc; ++i) {
                ...
                if ((pos = arg.find ("-co:")) == 0) {
                    ...
                    // Split arg based on '='
                    std::vector<std::string> co = mplot::tools::stringToVector (arg_ss, std::string("="));
                    if (co.size() >= 2) {
        ...
        T get (const std::string& thing, T defaultval) const
        {
            if (this->config_overrides.count(thing) > 0) {
                ...
                if constexpr (std::is_same<std::decay_t<T>, bool>::value == true) {
                    ...
                    std::cout << "Type not handled by mplot::Config::get, return default value: "
                              << defaultval << std::endl;
                }
                return rtn;
