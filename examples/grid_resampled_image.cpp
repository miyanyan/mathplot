...
int main()
{
    ...
    // Visualize resampled
    auto gv2 = std::make_unique<morph::GridVisual<float>>(&g2, sm::vec<float>{ 0.0f, -2.0f, 0.0f });
    v.bindmodel (gv2);
    ...
    auto hgv = std::make_unique<morph::HexGridVisual<float>>(&hg, sm::vec<float>{ g2.width() / 2.0f, g2.height() / 2.0f - 4.0f , 0.0f });
    v.bindmodel (hgv);
